<?php
/**
 * Created by PhpStorm.
 * User: barry
 * Date: 08/03/2017
 * Time: 11:15
 */
if(isset($_COOKIE["username"])) {
    ?>
    <!DOCTYPE html>
    <html>
    <head>
        <title>Student Records</title>
        <link rel="stylesheet" href="login.css">
        <!-- Latest compiled and minified CSS -->
        <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap.min.css">

        <!-- jQuery library -->
        <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.1.1/jquery.min.js"></script>

        <!-- Latest compiled JavaScript -->
        <script src="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/js/bootstrap.min.js"></script>
        <!-- Load jQuery from Google's CDN -->
        <!-- Load jQuery UI CSS  -->
        <link rel="stylesheet" href="http://code.jquery.com/ui/1.10.3/themes/smoothness/jquery-ui.css" />

    </head>
    <style>
        a:link,a:visited {
            text-decoration: none;
            color: black;}
    </style>
    <body>
    <header>
        <nav class="navbar navbar-default">
            <div class="container-fluid">
                <div class="navbar-header">
                    <img  src="data:image/jpeg;base64,">
                </div>
                <ul class="nav navbar-nav navbar-right">
                    <li><a href="home.php"><button class="btn btn-primary navbar-btn">Home</button></a></li>
                    <li> <a href="logout.php"><button class="btn btn-primary navbar-btn">Logout</button></a></li>
                </ul>
            </div>
        </nav>
    </header>
<?php
/**
 * Created by PhpStorm.
 * User: barry
 * Date: 13/03/2017
 * Time: 12:53
 */
require_once('conn.php');//open connection to the database
$date1 = $_POST['date1'];
$date2 = $_POST['date2'];

$moduleID = $_POST['module_id'];
$studentID = $_POST['student_id'];

Try{
    $sql = " select COUNT(*) FROM module_attendance where created_at >= :date1 AND created_at <= :date2  AND module_id = :moduleID AND student_id=:studentID";
    $stmt = $handler->prepare($sql);
    $stmt->bindParam(':date2', $date2);
    $stmt->bindParam(':date1', $date1);
    $stmt->bindParam(':moduleID', $moduleID);
    $stmt->bindParam(':studentID', $studentID);
    $stmt->execute();
    //echo $stmt->fetchColumn();
    $attendanceDays = $stmt->fetchColumn();

}
catch(PDOException $e) {
    echo $e;
}

//************************************************************//
//***** GET TOTAL NUMBER OF DAYS BETWEEN SELECTED DATES ******//
//************************************************************//
$start = new DateTime($date1);
$end = new DateTime($date2);

$end->modify('+1 day');
$start->modify('+1 day');

$interval = $end->diff($start);
//total days between dates
$days = $interval->days;
//(P1D equates to 1 day)
$period = new DatePeriod($start,new DateInterval('P1D'),$end);

foreach ($period as $dt){
    $curr = $dt->format('D');

    if($curr == 'Sat' || $curr == 'Sun' || $curr == 'Fri'){
        $days--;
    }
}
//echo $days;
//$totalDays = $days;

//Calculate Percentage of attendance
$attendancePercentage = round(($attendanceDays /$days )* 100,2);


?>
    <hr class='colorgraph'>;

    <form method="POST" action="attendance.php" name="Login_Form" class="form-signin">

        <h3 class="form-signin-heading">Attendance Record</h3>
        <hr class='colorgraph'>
        <label class="form-control">Student ID</label>
        <input id="student_Id" class="form-control" value="<?php echo htmlspecialchars($studentID); ?>"/><br>
        <label class="form-control">Module ID</label>
        <input id="module_Id" class="form-control" value="<?php echo htmlspecialchars($moduleID); ?>"/><br>
        <label class="form-control">Attendance Percentage for selected period</label>
        <input id="attendance" class="form-control" value="<?php echo htmlspecialchars($attendancePercentage); ?>%"/><br>
        <button class="btn btn-primary btn-lg btn-block" name="Submit" value="Login" type="Submit">Produce new report
        </button><br>
        </form>

    </body>
    </html>
    <?php
}