...
if(isset($_COOKIE["username"])) {
    ...
    $query = $handler->query('SELECT * FROM module');
    echo "<h2  align='center'>All Current Modules - Update Lecturer or Delete Module</h2>";
...
<th colspan=\"2\">OPTIONS</th>
    ...
    while($row = $query->fetch()) {
        echo("<form method=\"post\" action=\"updateModules.php\">  ");
        echo "<tr>";
        echo "<td><input type='text' name='moduleID' value='" . $row['module_id'] . "' /></td>";
        echo "<td><input type='text' name='name' value='" . $row['name'] . "' /></td>";
        echo "<td><input type='text' name='lecturer' value='" . $row['Lecturer_ID'] . "'/></td>";
        echo("<td><input type='submit' name='myBtn' id='update' value='Update' ></td>");

        echo('<td><button><a href="deleteModule.php?id=' . $row['name'] . '">Delete</a></button></td>');
        echo ('</form>');
        echo ('</tr>');
    }
    echo "</table>";
