...
$query = $handler->query('SELECT * FROM student');
echo "<h2 align='center'>All registered Student Details -- Edit or Delete Records Below</h2>";
...
while($row = $query->fetch()) {
    echo("<form method=\"post\" action=\"updateStudents.php\">  ");
    echo "<tr >";
    echo "<td><input type='text' name='studentID' value='" . $row['Student_ID'] . "' /></td>";
    echo "<td><input type='text' name='fname' value='" . $row['f_name'] . "' /></td>";
    echo "<td><input type='text' name='sname' value='" . $row['surname'] . "'/></td>";
    echo "<td><input type='text' name='mail' value='" . $row['email'] . "'/></td>";
    echo "<td><input type='text' name='pword' value='" . $row['password'] . "'/></td>";
    echo("<td><input type='submit' name='myBtn' id='update' value='Update' ></td>");
    echo('<td><button><a href="deleteStudent.php?id=' . $row['Student_ID'] . '">Delete</a></button></td>');
    echo "</form>";
    echo "</tr>";
}
echo "</table>";
