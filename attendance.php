<?php
/**
 * Created by PhpStorm.
 * User: barry
 * Date: 08/03/2017
 * Time: 11:15
 */
if(isset($_COOKIE["username"])) {
    ?>
    <!DOCTYPE html>
    <html>
    <head>
        <title>Student Records</title>
        <link rel="stylesheet" href="login.css">
        <!-- Latest compiled and minified CSS -->
        <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap.min.css">

        <!-- jQuery library -->
        <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.1.1/jquery.min.js"></script>

        <!-- Latest compiled JavaScript -->
        <script src="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/js/bootstrap.min.js"></script>
        <!-- Load jQuery from Google's CDN -->
        <!-- Load jQuery UI CSS  -->
        <link rel="stylesheet" href="http://code.jquery.com/ui/1.10.3/themes/smoothness/jquery-ui.css" />

        <!-- Load jQuery JS -->
        <script src="http://code.jquery.com/jquery-1.9.1.js"></script>
        <!-- Load jQuery UI Main JS  -->
        <script src="http://code.jquery.com/ui/1.10.3/jquery-ui.js"></script>

        <!-- Load SCRIPT.JS which will create datepicker for input field  -->
        <script src="script.js"></script>
    </head>
    <style>
        a:link,a:visited {
            text-decoration: none;
            color: black;}
    </style>
    <body>
    <header>
        <nav class="navbar navbar-default">
            <div class="container-fluid">
                <div class="navbar-header">
                    <img  src="data:image/jpeg;base64,">
                </div>
                <ul class="nav navbar-nav navbar-right">
                    <li><a href="home.php"><button class="btn btn-primary navbar-btn">Home</button></a></li>
                    <li> <a href="logout.php"><button class="btn btn-primary navbar-btn">Logout</button></a></li>
                </ul>
            </div>
        </nav>
    </header>
    <?php

    require_once('conn.php');//open connection to the database
    echo "<hr class='colorgraph'>";

    $query = $handler->query('SELECT student_ID FROM student');
    $query1 = $handler->query('SELECT module_id FROM module');

    echo('<form method="POST" action="attendancePercentage.php" name="Login_Form" class="form-signin">');

    echo ('<h3 class="form-signin-heading">Check Attendance</h3>');
    echo ("<hr class='colorgraph'>");
    echo "<select name='student_id'  class=\"form-control\" placeholder=\"Student ID\">";
    while($row = $query->fetch()) {
        echo '<option>'.$row['student_ID'].'</option>';
    }
    echo "</select><br>";
    echo "<select name='module_id'  class=\"form-control\" placeholder=\"Module ID\">";

    while($row = $query1->fetch()) {
        echo '<option>'.$row['module_id'].'</option>';
    }
    echo "</select><br>";
    echo  '<input name=date1 type="text" id="datepicker1"  placeholder="Start Date"  class="form-control" required=""/><br>';
    echo  '<input name=date2 type="text" id="datepicker2"  placeholder="End Date"  class="form-control" required=""/><br>';
    echo '<button class="btn btn-primary btn-lg btn-block" name="Submit" value="Login" type="Submit">Check Attendance
    </button><br>';
    echo '</form>';

    ?>

    </body>
    </html>
    <?php
}